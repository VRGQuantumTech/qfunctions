...
class Trace():
    ...
    def cab_load_trace(self, path = None):
        ...
        if path is None: path, file, dirpath = pth.file()
        else: file = path.split('/' )[-1]
        ...
        if np.mean(df[df.columns[2]]) == 0.0:
            ...
            if self.t.min() < 0: self.t = 10**(self.t/20)
            self.mod = 'amp'
        else: 
    ...
    def do_nonlfit(self, x0, residual = False, units = None, bound_tol = 1000,
...
def _guess_kappa_coupling(trace, verbose = True):
    ...
    if trace.mod == 'iq': t_amp = abs(trace.t)
    else: t_amp = trace.t
    ...
    if verbose: print(('\033[1;36m Aprox kc is %.2e Hz \033[0m'
                       %(kappac_aprox)))
...
def fit_res_com(trace, x0, mod='amp', jac='3-point', verbose=0, gtol = 1e-15,
    ...
    if model == 'parallel':
        if mod == 'amp':
            ...
            if unwrap: t = np.unwrap(np.arctan2(np.imag(trace.t),np.real(trace.t)))
            else: t = np.arctan2(np.imag(trace.t),np.real(trace.t))
        elif mod == 'real':
    ...
    if model == 'parallel':
        if mod == 'amp': x1[5], LER_position = _find_minimum(trace.f, t, x1[0])
        else: _, LER_position = _find_minimum(trace.f, t, x1[0])
    elif model == 'inline':
        if mod == 'amp': x1[5], LER_position = _find_maximum(trace.f, t, x1[0])
    ...
    if mod == 'amp': tprx = abs(lorentzian(trace.f, xf))
    if mod == 'phase': tprx = np.unwrap(
            np.arctan2(np.imag(lorentzian(trace.f, xf)),np.real(lorentzian(trace.f, xf))))
    if mod == 'real': tprx = np.real(lorentzian(trace.f, xf))
    if mod == 'imag': tprx = np.imag(lorentzian(trace.f, xf))

    if plot == True: fig = plot_results(trace.f, t, tprx, xf, LER_position, mod)
    else: fig = None
    
    res = _calculate_residual(t,tprx)
    
    
    return Results(trace.f, t, tprx, xf, res, jac, fig, mod)


#%% FUNCTIONS TO PLOT AND PRESENT THE RESULTS

def plot_results(f, t, tprx, xf, LER_position, mod, plot = False):
    ...
    print('fr = ' + str(format(xf[7], "E")) + ' Hz')
    Qi = xf[7]/(2*xf[4])
    Qc = xf[7]/(2*xf[5])
    print('Qi = ' + str(format(Qi, "E")))
    print('Qc = ' + str(format(Qc, "E")))

    
    if plot:
        
        uplt.plottoinline()
        order = mt.floor(mt.log10(np.abs(f.max())))
        
        fig = plt.figure('Result least squares')
        
        if order >= 9:
            plt.plot(f*1e-9,t, 'k', label = 'measurement', lw=3.5, zorder=2)
            plt.scatter(f[LER_position-1]*1e-9,t[LER_position], facecolors='white', edgecolors='k', zorder=1, s=100,label = 'S21 max')
            plt.plot(f*1e-9,tprx, 'r--', label = 'fit', lw=1.5, zorder=3)
            plt.xlabel('Frequency (GHz)' , fontsize = 14, fontweight='bold')
            plt.xlim([f.min()*1e-9, f.max()*1e-9])
        
        if order >= 6 and order < 9:
            plt.plot(f*1e-6,t, 'k', label = 'measurement', lw=2.5, zorder=2)
            plt.scatter(f[LER_position]*1e-6,t[LER_position], facecolors='gray', edgecolors='k', zorder=1, label = 'S21 max')
            plt.plot(f*1e-6,tprx, 'r--', label = 'fit', lw=1.5, zorder=3)
            plt.xlabel('Frequency (MHz)' , fontsize = 14, fontweight='bold')
            plt.xlim([f.min()*1e-6, f.max()*1e-6])
        
        if order >= 3 and order < 6:
            plt.plot(f*1e-3,t, 'k', label = 'measurement', lw=2.5, zorder=2)
            plt.scatter(f[LER_position]*1e-3,t[LER_position], facecolors='gray', edgecolors='k', zorder=1, label = 'S21 max')
            plt.plot(f*1e-3,tprx, 'r--', label = 'fit', lw=1.5, zorder=3)
            plt.xlabel('Frequency (kHz)' , fontsize = 14, fontweight='bold')
            plt.xlim([f.min()*1e-3, f.max()*1e-3])
        
        if order >= 0 and order < 3:
            plt.plot(f,t, 'k', label = 'measurement', lw=2.5, zorder=2)
            plt.scatter(f[LER_position],t[LER_position], facecolors='gray', edgecolors='k', zorder=1, label = 'S21 max')
            plt.plot(f,tprx, 'r--', label = 'fit', lw=1.5, zorder=3)
...
def fit_plot(trace, results = [], residual = False, xlim = None, save = False):
    ...
    if len(results) == 1 or dim[1] == 1: ax = np.reshape(ax, dim)
        ...
        for j in range(dim[1]):
            if j == 0:
                ax[i,j].plot(f*1e-9, results[i].t, 'ko', label = 'measurement', 
                             lw=3.5, zorder=2)
                ax[i,j].plot(f*1e-9, results[i].tprx, 'r--', label = 'fit', lw=2.5, zorder=3)
    ...
    if xlim != None and type(xlim) == tuple: plt.xlim(xlim)
...
def fit_nonl_res(trace, x0, bound_tol=1000, jac='3-point', verbose=0,
    ...
    if plot == True: fig = plot_nonl_results(trace, tprx, xf, LER_position,
                                             mod = 'amp', save = save)
    else: fig = None
    
    res = _calculate_residual(t,tprx)
    
    return Results(trace.f, t, tprx, xf, res, jac, fig, mod = 'amp')
...
def _check_roots(roots):
    if np.isrealobj(roots):
        return roots, True
    else: return roots[0].real, False
...
def plot_nonl_results(trace, tprx, xf, LER_position, mod,
...
def power_sweep_analysis(jac ='2-point', mode = 'amp', units = 'linear',
    ...
    for i in trace:
        ...
        x0 = np.array([_guess_amplitude(trace[i].t),
                   _guess_phase_delay(trace[i].f,trace[i].t, unwrap = False),
    ...
    for i in fit_results:
        ...
        numphot[i]=conv.Ptophotons(Pwat[i],Qc[i]*Qi[i]/(Qc[i]+Qi[i]),2*np.pi*fr[i],Qc[i])
